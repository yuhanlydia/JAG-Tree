...
def recursive_credit(root: TreeNode, baseline: Callable[[TreeNode], float] | float = 0.0) -> tuple[float, np.ndarray]:
    """Public unique-edge recursive credit contract.

    This name exposes the preserved Phase-0 estimator to model-backed code;
    :func:`recursive_estimate` remains as a compatibility name for audits.
    """

    return recursive_estimate(root, baseline)
